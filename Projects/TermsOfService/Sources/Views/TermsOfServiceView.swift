    ...
    public var body: some View {
        VStack(alignment: .leading) {
            ...
            VStack(alignment: .leading, spacing: 20) {
                makeCheckView(isCheck: store.isAllCheck, text: "전체 동의", isAllCheckView: true) {
                    store.send(.allCheckButtonTapped)
                }
                GrayDivider()
                makeCheckView(isCheck: store.isServiceCheck, text: "[필수] 서비스 이용약관 동의") {
                    store.send(.serviceCheckButtonTapped)
                }
                makeCheckView(isCheck: store.isInfoCheck, text: "[필수] 개인정보 수집·이용 동의") {
                    store.send(.infoCheckButtonTapped)
                }
                makeCheckView(isCheck: store.isAgeCheck, text: "[필수] 만 14세 이상 회원입니다.") {
                    store.send(.ageCheckButtonTapped)
                }
            }
            Spacer()
            RectangleBottomButton(isEnable: $store.isPossibleNext, text: "다음") {
