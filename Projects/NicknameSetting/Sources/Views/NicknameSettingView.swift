...
public struct NicknameSettingView: View {
    ...
    public var body: some View {
        VStack(alignment: .leading) {
            ...
            VStack(spacing: 6) {
                HStack(spacing: 6) {
                    Image(asset: store.isValidCount ? CommonAsset.checkIconGreen14px : CommonAsset.checkIconGray14px)
                ...
                HStack(spacing: 6) {
                    Image(asset: store.isValidRegex ? CommonAsset.checkIconGreen14px : CommonAsset.checkIconGray14px)
            ...
            Spacer()
            RectangleBottomButton(isEnable: $store.isPossibleNickname, text: "회원가입 완료") {
