...
public struct ShowDetailView: View {
    ...
    public var body: some View {
        VStack(spacing: 0) {
            ...
            ScrollView {
                VStack(spacing: 0) {
                    VStack(spacing: 0) {
                        ...
                        LazyImage(url: URL(string: store.showInfo?.poster ?? "")) { state in
                        ...
                        Text(store.showInfo?.genre.nameKR ?? "")
            ...
            .onAppear {
                ...
                store.send(.fetchDetailResponse)
            }
            .onDisappear {
    ...
    private var rankView: some View {
        HStack {
            ...
            Text(String(format: "%.1f", (store.showInfo?.reviewGradeAvg ?? 0)))
                .font(.body4)
                .foregroundStyle(Color.primary1)
            Spacer().frame(width: 6)
            Text("(\(store.showInfo?.reviewCount ?? 0)개 리뷰)")
                .font(.body4)
        ...
        HStack(spacing: 0) {
            ForEach(ShowDetailFeature.ShowDetailCategoryType.allCases, id: \.self) { type in
                VStack {
                    ...
                    Text(type.rawValue)
                        .font(.subTitle4)
                        .foregroundStyle(store.currentSelectedCategory == type ? Color.primary1 : Color.gray6)
                    Spacer()
                    if store.currentSelectedCategory == type {
                        Color.primary1
                            .frame(height: 3)
                    } else {
                        Color.gray6
    ...
    private var detailTapView: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack {
                ...
                Group {
                    Text((store.showInfo?.startDate)?.replacingOccurrences(of: "-", with: ".") ?? "")
                    + Text(" - ")
                    + Text((store.showInfo?.endDate)?.replacingOccurrences(of: "-", with: ".") ?? "")
                }
            ...
            HStack {
                makeDetailTapTitleView(title: "티켓가격")
                    .padding([.leading, .vertical], 6)
                Text(store.showInfo?.ticketPrice ?? "")
                    .padding(.horizontal, 12)
                    .font(.body3)
                    .foregroundStyle(Color.gray2)
                Spacer()
            ...
            HStack {
                makeDetailTapTitleView(title: "공연장소")
                    .padding([.leading, .vertical], 6)
                Text(store.showInfo?.facilityName ?? "")
                    .padding(.horizontal, 12)
                    .font(.body3)
                    .foregroundStyle(Color.gray2)
                Spacer()
            ...
            HStack {
                makeDetailTapTitleView(title: "주소")
                    .padding([.leading, .vertical], 6)
                Text(store.facilityInfo?.address ?? "")
                    .padding(.horizontal, 12)
                    .font(.body3)
                    .foregroundStyle(Color.gray2)
                Spacer()
            ...
            HStack {
                makeDetailTapTitleView(title: "웹사이트")
                    .padding([.leading, .vertical], 6)
                Text(store.facilityInfo?.homepage ?? "")
                    .padding(.horizontal, 12)
                    .font(.body3)
                    .foregroundStyle(Color.gray2)
                Spacer()
            ...
            makeDetailImages(images: store.showInfo?.introductionImages ?? [])
        }
